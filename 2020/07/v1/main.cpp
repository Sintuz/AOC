...
void printBagTree(Bag *bag, int depth) {
    ...
    if(contents->size()!=0) {
        for(ContentBag sbag : *contents) {
            cout << string(depth*2, ' ') + "- " << sbag.getNum() << " " << sbag.getColor() << endl;
            Bag *b = findBag(sbag.getColor());
            printBagTree(b, depth+1);
        }
    }
}
/*
